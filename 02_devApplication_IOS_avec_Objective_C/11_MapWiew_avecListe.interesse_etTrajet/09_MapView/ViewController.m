...
//definir un tableau pour stocker les résultats de la recherche (afin de pouvoir les envoyer vers le 2e controleur )
...
{
    //supprimer les placemarks des anciennes recherches
    [self.mapView removeAnnotations:[self.mapView annotations]];
    
    //supprimer le tableau contenant les resultats de la recherche précedente
    ...
    [recherche startWithCompletionHandler:^(MKLocalSearchResponse * _Nullable response, NSError * _Nullable error)
    {
        ...
        {
            // je veux stocker les éléments trouvés dans mon tableau tbRésultats pour les transmetre au 2em controlleur
...
//interdire la transition si le tableau de résultat ne contient rien
//Pour cela on override la méthode
-(BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender{
    //comme on peut avoir plusieurs segues qui partent du même controleur, je dois (je devrais) tester quel est le segue passe en parametre (effectivement, cette methode est appeler pour toute les segues)
    
    //remarque: dans mon cas ce test n'est pas vraiment necessaire car il n'y a qu'un seul segue au depart de se controleur
    
    if ([identifier isEqualToString:@"segueVersResultatRecherche"]) {
        //je voudrai interdire cette transition si il n'y a pas de resultat a afficher
        if(self.tbResultats != nil && self.tbResultats.count > 0)
            return YES;
        else{
            [self afficherAlerteAvecTitre:@"Afficher détails" etMessage:@"Il n'y a pas de résultat à détailler"];
            return NO;
        }
    } else{
        return YES;
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    //recuperer le destinataire de la transition
    ResultatRechercheViewController* destinataire = (ResultatRechercheViewController*)[segue destinationViewController];
    
    destinataire.itemTrouves = self.tbResultats;
}
